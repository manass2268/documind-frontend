    ...
    <>
      ...
      <div className="bg-white border-b border-gray-200 py-1.5 px-2 sm:px-4 md:px-8 text-[10px] sm:text-xs font-medium text-gray-700 flex flex-wrap justify-between items-center relative z-50">
        <div className="flex items-center gap-1 sm:gap-2 w-full md:w-auto justify-center md:justify-start mb-1 md:mb-0">
          ...
          <span className="tracking-wide text-gray-900 font-bold text-[10px] sm:text-[11px] md:text-[13px]">भारत सरकार <span className="font-normal text-gray-400 mx-0.5 sm:mx-1">|</span> Government of India</span>
        </div>
        ...
        <div className="hidden md:flex items-center gap-2 lg:gap-4 text-[10px] lg:text-[12px] flex-wrap justify-center">
          ...
          <span className="text-gray-300">|</span>
          <button onClick={() => setLanguage('en')} className={`flex items-center gap-0.5 lg:gap-1 font-bold transition-colors ${language === 'en' ? 'text-[#0056D2]' : 'text-black hover:text-[#0A4275]'}`}>English <ChevronDown size={12} className="lg:w-[14px] lg:h-[14px]"/></button>
          <span className="text-gray-300">|</span>
      ...
      <nav className="bg-gradient-to-r from-[#002244] via-[#003366] to-[#004080] text-white shadow-md relative z-50">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-8">
          <div className="flex justify-between items-center h-12 md:h-14">
            <button className="lg:hidden p-2 -ml-2 text-white hover:bg-white/10 rounded-md transition-colors" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="lg:hidden overflow-hidden bg-[#001f3f] border-t border-white/10">
