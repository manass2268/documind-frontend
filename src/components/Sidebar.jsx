...
import { Brain, Plus, FileText, Settings, X, HelpCircle } from "lucide-react";

export default function Sidebar({ isOpen, closeSidebar }) {
  return (
    <>
      {/* Dark Overlay - Ab yeh har screen par kaam karega */}
      {isOpen && (
        <div className="fixed inset-0 bg-black/60 z-40 backdrop-blur-sm transition-opacity" onClick={closeSidebar} />
      )}
      
      {/* Sidebar Drawer - Always Fixed & Hidden by default */}
      <div className={`
        fixed inset-y-0 left-0 z-50 w-72 h-full backdrop-blur-xl bg-[#0A0A0F]/95 border-r border-white/10 p-5 flex flex-col shadow-2xl transition-transform duration-300 ease-in-out
        ${isOpen ? "translate-x-0" : "-translate-x-full"}
      `}>
        
        {/* Close Button - Ab yeh har screen par dikhega */}
        <button className="absolute top-5 right-5 text-gray-400 hover:text-white transition-colors" onClick={closeSidebar}>
          <X size={24} />
        </button>
        ...
        <div className="flex-1 overflow-y-auto space-y-1 -mx-2 px-2 scrollbar-hide">
          {[
            ...
            <div key={i} className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 cursor-pointer transition-colors group">
              <FileText size={16} className="text-gray-400 group-hover:text-purple-400 transition-colors shrink-0" />
        ...
        <div className="mt-auto pt-4 border-t border-white/10 space-y-2">
          <Link to="/support" className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-white/5 cursor-pointer transition-colors text-gray-400 hover:text-white group">
          ...
          <div className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-white/5 cursor-pointer transition-colors text-white">
            <div className="h-10 w-10 rounded-full bg-gradient-to-tr from-purple-600 to-cyan-600 flex items-center justify-center text-sm font-bold shadow-lg shrink-0">MS</div>
            <div className="flex-1 min-w-0">
              <div className="text-sm font-medium truncate">Manas Singh</div>
            ...
            <Settings size={18} className="text-gray-400 hover:text-white shrink-0" />
          </div>
        </div>
      </div>
    </>
