      ...
      <section className="bg-gradient-to-br from-[#EBF4FF] via-white to-[#F0F7FF] pt-20 pb-24 relative overflow-hidden border-b border-gray-100">
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10">
          <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="z-20">
            ...
            <motion.div variants={fadeInLeft} whileHover={{ scale: 1.05 }} className="inline-flex items-center gap-2.5 text-[12px] font-bold text-gray-700 bg-white shadow-md shadow-blue-500/10 w-max px-4 py-2.5 rounded-xl border border-gray-200 cursor-default">
            ...
            {policySections.map((section) => (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  {section.bullets && (
                    <div className="space-y-3 mt-4 bg-slate-50/80 p-5 rounded-2xl border border-slate-100 transition-colors">
                      {section.bullets.map((bullet, i) => (
                        <div key={i} className="flex items-start gap-3 group/bullet">
                          <div className={`p-1 rounded-full ${getBulletColor(section.color)} shrink-0 mt-0.5 shadow-sm`}>
                            <ShieldCheck size={10} className="text-white" />
                          </div>
                          <span className="text-[13px] text-gray-700 font-semibold">{bullet}</span>
                        </div>
                      ))}
                    </div>
                  )}
